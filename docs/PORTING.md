...
## No Perplexity branding — the app must be AI/LLM independent

The original was generated by Perplexity and contains references to it. NONE may survive the
port: no links to perplexity.ai or *.pplx.app, no "via Perplexity Computer" labels, no
"Generated by …" export footers mentioning pplx.app (bundle lines 28952, 29744, 29770).
Replace share/export attribution with just "Wanderlust Planner". The app has no AI features
and must not call any LLM/AI API.
